...
const randomColorGenerator = ()=>{
    var firstNumber = Math.floor(Math.random()*256)
    var secondNumber = Math.floor(Math.random()*256)
    var thirdNumber = Math.floor(Math.random()*256)

    var randomColor = 'rgb('+firstNumber+','+secondNumber+','+thirdNumber+')'
    return randomColor
}

function displayQuote(){
    var quotes = randomQuoteGenerator()
    //var quoteContainer = document.querySelector('.quote')
    var quoteText = document.querySelector('.card-title')
    quoteText.innerHTML = quotes.quote
    var quoteCitation = document.querySelector('.card-footer')
    quoteCitation.innerHTML = quotes.source
    document.body.style.backgroundColor = randomColorGenerator()
    //var quoteText = `<h3>${quotes.quote}</h3>`
}
